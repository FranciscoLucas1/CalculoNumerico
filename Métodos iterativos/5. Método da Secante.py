...
def menu():
    print("\033[1mAlgoritmo: Método da Secante\033[m")
...
def instrucoes():
    i = (
        ...
        "Este programa permite calcular raízes de funções usando o método da Secante.\n"
...
while True:
    ...
    match opcao:
        case "1":
            ...
            xi = 0.0
            x = list() # lista com os valores do x atual e do anterior x
            n = 1 # índice dos valores de x (n é valor atual e n-1 é valor anterior)
            ...
            while True:
                ...
                xi = x[n] - (x[n] - x[n-1])/(func(x[n], f) - func(x[n-1], f))*func(x[n], f)
                
                x.append(xi)
                n+=1
                i+=1
                
                if math.fabs(func(xi, f)) < p or i > 100:
                    dados_resultados.append([i, xi, func(xi, f)])
                    print(f"   {i:2d}    | {xi:12.6f} | {func(xi,f):14.6f} |")
                    break
