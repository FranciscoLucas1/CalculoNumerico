...
def menu():
    print("\033[1mAlgoritmo: Método do Ponto Fixo\033[m")
...
def instrucoes():
    i = (
        ...
        "Este programa permite calcular raízes de funções usando o método do ponto fixo.\n"
        "Você pode escolher entre diferentes funções matemáticas, como seno, cosseno, tangente, logaritmo etc.\n"
        "Digite a função desejada, os valores iniciais, a função de iteração e a precisão desejada (em notação científica ou formato decimal).\n"
...
while True:
    ...
    match opcao:
        case '1':
            ...
            phi = input("Digite a função de iteração: ")
            ...
            print("-" * 65)
            cabecalho_info = ["Informações:", f"Função escolhida: {f}", f"Função de iteração: {phi}", f"Valor inicial (x0): {x0}", f"Precisão: {p}"]
            ...
            while True:
                ...
                dados_resultados.append([i, xi, f_xi])
                print(f"   {i:2d}    | {xi:12.6f} | {f_xi:14.6f} |")
                if math.fabs(xi - x0) < p:
                    break
                x0 = xi
            ...
            print(f"\033[1mNúmero de iterações: {i}\033[m")
